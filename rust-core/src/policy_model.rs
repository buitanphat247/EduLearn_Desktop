...
impl ExamPolicy {
    ...
    pub fn validate_for(&self, expected_exam_id: &str, now_ms: u64) -> Result<(), String> {
        ...
        let mut prohibited = normalized_set(&self.blocked_processes);
        prohibited.extend(normalized_set(&self.remote_processes));
        prohibited.extend(normalized_set(&self.screen_capture_processes));
        prohibited.extend(normalized_set(&self.debug_processes));
        if !self.allow_vm {
            prohibited.extend(normalized_set(&self.virtual_machine_processes));
        }
...
mod tests {
    use super::{
        ExamPolicy, MAX_POLICY_LIFETIME_MS, REMEDIATION_FAILURE_CONTINUE_AND_AUDIT,
        REMEDIATION_FAILURE_RECOVERY_REQUIRED,
    };
    ...
    fn rejects_conflicting_or_path_based_process_entries() {
        let mut policy = ExamPolicy::strict_builtin();
        policy.allowed_processes = vec!["obs64.exe".to_string()];
